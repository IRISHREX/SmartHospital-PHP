...
class Patient_model extends MY_Model
{
    ...
    public function add_opd($data, $transcation_data, $charge, $opd_visit_data)
    {
        if (isset($data['id']) && $data['id'] != '') {
            ...
            $opd_id = $data['id'] ;			
			$message = UPDATE_RECORD_CONSTANT . " On Opd Details id " . $data['id'] ;
            $action = "Update";             
            $this->log($message, $opd_id, $action);
    ...
    public function add_discharge($data)
    {
        ...
        if (!empty($data['ipd_details_id'])) {
            ...
            // insert log recored//
            $message = UPDATE_RECORD_CONSTANT . " On IPD Details id " . $data['ipd_details_id'];
            $action = "Update";
            $record_id = $data['ipd_details_id'];
            $this->log($message, $record_id, $action); 
            // ======================
            ...
            $this->db->update('bed', $bed_status);
            // insert log recored//
            $message = UPDATE_RECORD_CONSTANT . " On Bed id " . $bed['bed'];
            $action = "Update";
            $record_id = $bed['bed'];
            $this->log($message, $record_id, $action); 
            // ======================
        ...
        if (!empty($data['opd_details_id'])) {
            $opd_details_id=$data['opd_details_id'];
        ...
        if (isset($data['id']) && $data['id'] != '') {
            ...
            // insert log recored//
            $message = UPDATE_RECORD_CONSTANT . " On Discharge Card id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
        } else {
            ...
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On Discharge Card id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
        }
    ...
    public function getAllopdRecord()
    {
       ...
       if (!empty($custom_fields)) {
        ...
        $this->datatables
            ...
            ->join('staff as generated_by_staff', 'generated_by_staff.id = visit_details.generated_by', "left")			
    ...
    public function getAllopdvisitRecord($patientid)
    {
        ...
        $this->datatables
            ...
            ->join('patient_charges', 'opd_details.id=patient_charges.opd_id', 'left')
            ->join('staff as generated_by_staff', 'generated_by_staff.id = opd_details.generated_by', "left")
            ...
            ->orderable('opd_details.id,opd_details.case_reference_id,visit_details.appointment_date,staff.name,visit_details.refference,visit_details.symptoms,
                generated_by_staff.name as generated_byname,'."$custom_field_column".'')
    ...
    public function getAllipdRecord()
    {
        ...
        if ($doctor_restriction == 'enabled') {
            if ($userdata["role_id"] == 3) {
                ...
                ->where('ipd_details.cons_doctor', $userdata['id'])
                ->or_where('ipd_doctors.consult_doctor', $userdata['id'])
                ->group_end();
            }
        }
        $field_variable = (empty($field_var_array))? "": ",".implode(',', $field_var_array);
        $custom_field_column = (empty($custom_field_column_array))? "": ",".implode(',', $custom_field_column_array);
        $this->datatables
            ->select('patients.*,bed.name as bed_name,bed_group.name as bedgroup_name, floor.name as floor_name,ipd_details.date,ipd_details.id as ipdid,ipd_details.case_reference_id,ipd_details.credit_limit as ipdcredit_limit,ipd_details.case_type,staff.name,staff.surname,staff.employee_id, generated_by_staff.name as generated_byname,generated_by_staff.surname as generated_bysurname,generated_by_staff.employee_id as generated_byemployee_id' . $field_variable)
            ->join('patients', 'patients.id = ipd_details.patient_id', "inner")
			->join('ipd_doctors', 'ipd_doctors.ipd_id = ipd_details.id', "left")
            ->join('staff as generated_by_staff', 'generated_by_staff.id = ipd_details.generated_by', "left")
            ...
            ->searchable('patients.patient_name,ipd_details.id,ipd_details.case_reference_id,patients.id,patients.gender,patients.mobileno,staff.name,bed.name' . $custom_field_column.',ipd_details.credit_limit as ipdcredit_limit')
            ->orderable('patients.patient_name,ipd_details.case_reference_id,ipd_details.id,patients.id,patients.gender,patients.mobileno,staff.name,bed.name' . $custom_field_column.',"",ipd_details.credit_limit as ipdcredit_limit')
    ...
    public function getAlldischargedRecord()
    {
        ...
        $this->datatables
            ->select('patients.*,"0" charges,"0" othercharge,"0" tax,"0" discount,bed.name as bed_name,bed_group.name as bedgroup_name, floor.name as floor_name,ipd_details.date,ipd_details.id as ipdid,ipd_details.credit_limit as ipdcredit_limit,ipd_details.case_type,staff.name,staff.surname,staff.employee_id,ipd_details.case_reference_id,discharge_card.discharge_date,generated_by_staff.name as generated_byname,generated_by_staff.surname as generated_bysurname,generated_by_staff.employee_id as generated_byemployee_id
             ')
    ...
    public function add_nursenote($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            $message = UPDATE_RECORD_CONSTANT . " On Nurse Note id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
            ...
            $message = INSERT_RECORD_CONSTANT . " On Nurse Note id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action); 
        }
    ...
    public function deleteOPD($id,$patient_id)
    {
        ...
        // Orphan cleanup: OPD prescriptions hang off visit_details (not opd_details) and there is no
        // DB cascade from opd_details -> visit_details, so deleting the OPD alone leaves the
        // prescription subtree + visit rows orphaned. Remove them here, inside this same transaction
        // (atomic with the OPD delete — any failure rolls everything back). Physical prescription
        // files are released from storage quota + unlinked by the controller BEFORE this runs.
        // NOTE: deeper visit_details dependents (zoom conference, patient_charges) are intentionally
        // left untouched — out of scope for prescription-orphan cleanup.
        $saas_visit_ids = array();
        $saas_visits    = $this->db->select('id')->from('visit_details')->where('opd_details_id', $id)->get()->result_array();
        foreach ($saas_visits as $saas_visit) {
            $saas_visit_ids[] = $saas_visit['id'];
        }
        if (!empty($saas_visit_ids)) {
            $saas_basic_ids = array();
            $saas_basics    = $this->db->select('id')->from('ipd_prescription_basic')->where_in('visit_details_id', $saas_visit_ids)->get()->result_array();
            foreach ($saas_basics as $saas_basic) {
                $saas_basic_ids[] = $saas_basic['id'];
            }
            if (!empty($saas_basic_ids)) {
                $this->db->where_in('ipd_prescription_basic_id', $saas_basic_ids)->delete('ipd_prescription_test');
                $this->db->where_in('basic_id', $saas_basic_ids)->delete('ipd_prescription_details');
                $this->db->where_in('id', $saas_basic_ids)->delete('ipd_prescription_basic');
            }
            $this->db->where('opd_details_id', $id)->delete('visit_details');
        }
    ...
    public function deleteOPDPatient($id)
    {
        ...
        // Orphan cleanup: mirror deleteOPD. OPD prescriptions hang off visit_details (not
        // opd_details) and there is no DB cascade from opd_details -> visit_details, so deleting the
        // OPD alone would leave the prescription subtree + visit rows orphaned. Remove them here,
        // atomically (any failure rolls everything back). Physical prescription/transaction files are
        // released from storage quota + unlinked by the controller BEFORE this runs. Deeper
        // visit_details dependents (zoom conference, patient_charges) are intentionally left
        // untouched — out of scope for prescription-orphan cleanup.
        $saas_visit_ids = array();
        $saas_visits    = $this->db->select('id')->from('visit_details')->where('opd_details_id', $id)->get()->result_array();
        foreach ($saas_visits as $saas_visit) {
            $saas_visit_ids[] = $saas_visit['id'];
    ...
    // Lightweight read-only getter for SaaS storage release on IPD delete. Returns the
    // IPD's prescription rows that carry a file attachment. ipd_prescription_basic has an
    // ON DELETE CASCADE FK to ipd_details, so deleting the IPD removes these rows at the DB
    // level — their files must be released + unlinked first or they leak. Files live under
    // uploads/prescription_document.
    public function getIpdPrescriptionAttachments($ipdid)
    {
        return $this->db->select('id, attachment')
            ->where('ipd_id', $ipdid)
            ->where('attachment !=', '')
    ...
    // Lightweight read-only getter for SaaS storage release on OPD delete. Returns the OPD's
    // prescription rows that carry a file attachment. OPD prescriptions are physically stored in
    // ipd_prescription_basic (same table as IPD — see Prescription_model::add_ipdprescription),
    // distinguished by having visit_details_id set. Linked to this OPD via
    // ipd_prescription_basic.visit_details_id -> visit_details.opd_details_id. Files live under
    // uploads/prescription_document and must be released + unlinked before the OPD is deleted.
    public function getOpdPrescriptionAttachments($opdid)
    {
        return $this->db->select('ipd_prescription_basic.id, ipd_prescription_basic.attachment')
            ->from('ipd_prescription_basic')
            ->join('visit_details', 'visit_details.id = ipd_prescription_basic.visit_details_id', 'inner')
            ->where('visit_details.opd_details_id', $opdid)
            ->where('ipd_prescription_basic.attachment !=', '')
            ->get()
            ->result_array();
    }

    // Read-only getter for SaaS storage release on single-visit delete (deleteVisit). Returns every
    // prescription of this visit that carries a file attachment, keyed directly by visit_details_id.
    // Deliberately does NOT join ipd_prescription_details (medicines) — unlike the shared
    // Prescription_model::getopdvisitPrescription, which INNER-joins medicines and so silently drops
    // document-only prescriptions (those with only pathology/radiology). Files live under
    // uploads/prescription_document.
    public function getVisitPrescriptionAttachments($visit_id)
    {
        return $this->db->select('id, attachment')
            ->from('ipd_prescription_basic')
            ->where('visit_details_id', $visit_id)
            ->where('attachment !=', '')
            ->get()
            ->result_array();
    }
    ...
    public function addvisitDetails($opd_data)
    {
        if (isset($opd_data["id"])) {
			...
			$message = UPDATE_RECORD_CONSTANT . " On Visit Details id " .$opd_data["id"] ;
            $action = "Update";            
            $this->log($message, $opd_data["id"], $action);			
        } else {
    ...
    public function deleteVisit($id)
    {
        ...
        // Orphan cleanup: deleting the visit leaves its prescription subtree (which hangs off
        // visit_details_id) orphaned — there is no DB cascade from visit_details. Remove it here,
        // atomically (any failure rolls everything back), mirroring deleteOPD. Physical prescription
        // files are released from storage quota + unlinked by the controller BEFORE this runs.
        $saas_basic_ids = array();
        $saas_basics    = $this->db->select('id')->from('ipd_prescription_basic')->where('visit_details_id', $id)->get()->result_array();
        foreach ($saas_basics as $saas_basic) {
            $saas_basic_ids[] = $saas_basic['id'];
        }
        if (!empty($saas_basic_ids)) {
    ...
    public function getDetailsByCaseId($case_id,$type = null)
    {
       ...
       if(!empty($ipd_details)){
            ...
            $patient_details['organisation_id'] =$ipd_details['organisation_id'];
       ...
       if(!empty($opd_details)){
            ...
            $patient_details['organisation_id'] =$opd_details['organisation_id'];
    ...
    {
        return $this->db->select('patients.id as patient_id,patients.patient_name,patients.dob,patients.age,patients.month,patients.day,patients.image,patients.mobileno,patients.email,patients.gender,patients.blood_group,patients.address,patients.guardian_name,patients.is_dead,patients.insurance_id,patients.insurance_validity,patients.organisation_id,opd_details.id as opdid,opd_details.discharged,visit_details.appointment_date,organisation.organisation_name,patients.as_of_date')->from('patients')
    ...
    {
        return $this->db->select('patients.id as patient_id,patients.patient_name,patients.dob,patients.age,patients.month,patients.day,patients.image,patients.mobileno,patients.email,patients.gender,patients.blood_group,patients.address,patients.guardian_name,patients.is_dead,patients.insurance_id,patients.insurance_validity,patients.organisation_id,ipd_details.id as ipdid,`ipd_details`.`date`,ipd_details.discharged,ipd_details.credit_limit,ipd_details.date,bed.name as bed_name,bed.id as bed_id,bed_group.name as bedgroup_name,floor.name as floor_name,organisation.organisation_name,patients.as_of_date')->from('patients')
    ...
    public function getReferenceByIpdId($id)
    {
        $result = $this->db
            ...
            ->get("ipd_details")
            ->row();
        return $result ? $result->case_reference_id : null;
    }
    ...
    public function getPatientRadiologyVisitDetails($patient_id)
    {
        $this->db
			...
			->order_by("radiology_billing.id","desc")
			->where('patients.id',$patient_id); 
			$result= $this->db->get('radiology_billing');
			return $result->result_array();
    }

    public function getPatientBloodBankVisitDetails_old($patient_id) 
    {
        $blood_issue_query="SELECT `blood_issue`.*, IFNULL((SELECT sum(transactions.amount) from transactions WHERE transactions.blood_issue_id= blood_issue.id ), 0) as `paid_amount`, `blood_bank_products`.`name` as `blood_group`, `patients`.`patient_name`, `patients`.`gender`, `blood_donor`.`donor_name`, `blood_donor_cycle`.`bag_no`, `blood_donor_cycle`.`volume`, `blood_donor_cycle`.`unit`,charge_units.unit as `charge_unit` FROM `blood_issue` JOIN `patients` ON `patients`.`id` = `blood_issue`.`patient_id` JOIN `blood_donor_cycle` ON `blood_donor_cycle`.`id` = `blood_issue`.`blood_donor_cycle_id` JOIN `blood_donor` ON `blood_donor_cycle`.`blood_donor_id` = `blood_donor`.`id` JOIN `blood_bank_products` ON `blood_bank_products`.`id` = `blood_donor`.`blood_bank_product_id` left join charge_units on blood_donor_cycle.unit = charge_units.id	where `blood_issue`.`patient_id`=".$this->db->escape($patient_id)." ORDER BY `blood_issue`.`id` desc, `blood_issue`.`id` ";
        $blood_issue_sql        = $this->db->query($blood_issue_query);
        $blood_issue_result     = $blood_issue_sql->result_array();
        $component_issue_query="select blood_issue.*,IFNULL((SELECT sum(transactions.amount) from transactions WHERE transactions.blood_issue_id= blood_issue.id ),0) as `paid_amount`,blood_group.name as blood_group_name,component.name as component_name,patients.patient_name,patients.gender,blood_donor.donor_name,blood_donor_cycle.bag_no,blood_donor_cycle.volume,blood_donor_cycle.unit,charge_units.unit as `charge_unit` from blood_issue inner join blood_donor_cycle on blood_donor_cycle.id=blood_issue.blood_donor_cycle_id join blood_donor_cycle as bcd on blood_donor_cycle.blood_donor_cycle_id=bcd.id join blood_donor on blood_donor.id=bcd.blood_donor_id join blood_bank_products as component on component.id=blood_donor_cycle.blood_bank_product_id join blood_bank_products as blood_group on blood_group.id=blood_donor.blood_bank_product_id join patients on patients.id = blood_issue.patient_id left join charge_units on blood_donor_cycle.unit = charge_units.id where `blood_issue`.`patient_id`=".$this->db->escape($patient_id)." ORDER BY blood_issue.id desc";
    ...
    public function getPatientBloodBankVisitDetailsReport($patient_id) 
    {
        $blood_issue_query="SELECT `blood_issue`.*, IFNULL((SELECT sum(transactions.amount) from transactions WHERE transactions.blood_issue_id= blood_issue.id and transactions.patient_id = blood_issue.patient_id), 0) as `paid_amount`, `blood_bank_products`.`name` as `blood_group`, `patients`.`patient_name`, `patients`.`gender`, `blood_donor`.`donor_name`, `blood_donor_cycle`.`bag_no`, `blood_donor_cycle`.`volume`, `blood_donor_cycle`.`unit`,charge_units.unit as `charge_unit` FROM `blood_issue` JOIN `patients` ON `patients`.`id` = `blood_issue`.`patient_id` JOIN `blood_donor_cycle` ON `blood_donor_cycle`.`id` = `blood_issue`.`blood_donor_cycle_id` JOIN `blood_donor` ON `blood_donor_cycle`.`blood_donor_id` = `blood_donor`.`id` JOIN `blood_bank_products` ON `blood_bank_products`.`id` = `blood_donor`.`blood_bank_product_id` left join charge_units on blood_donor_cycle.unit = charge_units.id	where `blood_issue`.`patient_id`=".$this->db->escape($patient_id)." ORDER BY `blood_issue`.`id` desc, `blood_issue`.`id` ";
        $blood_issue_sql        = $this->db->query($blood_issue_query);
        $blood_issue_result     = $blood_issue_sql->result_array();
        $component_issue_query="select blood_issue.*,IFNULL((SELECT sum(transactions.amount) from transactions WHERE transactions.blood_issue_id= blood_issue.id and transactions.patient_id = blood_issue.patient_id),0) as `paid_amount`,blood_group.name as blood_group_name,component.name as component_name,patients.patient_name,patients.gender,blood_donor.donor_name,blood_donor_cycle.bag_no,blood_donor_cycle.volume,blood_donor_cycle.unit,charge_units.unit as `charge_unit` from blood_issue inner join blood_donor_cycle on blood_donor_cycle.id=blood_issue.blood_donor_cycle_id join blood_donor_cycle as bcd on blood_donor_cycle.blood_donor_cycle_id=bcd.id join blood_donor on blood_donor.id=bcd.blood_donor_id join blood_bank_products as component on component.id=blood_donor_cycle.blood_bank_product_id join blood_bank_products as blood_group on blood_group.id=blood_donor.blood_bank_product_id join patients on patients.id = blood_issue.patient_id left join charge_units on blood_donor_cycle.unit = charge_units.id where `blood_issue`.`patient_id`=".$this->db->escape($patient_id)." ORDER BY blood_issue.id desc";
        $component_issue_sql        = $this->db->query($component_issue_query);
        $component_issue_result     = $component_issue_sql->result_array();
         return array('blood_issue'=>$blood_issue_result,'component_issue'=>$component_issue_result);
    }
    ...
    {      
        $this->db
            ...
            ->where('patients.id',$patient_id)->order_by("ambulance_call.id","desc"); 
    ...
    {
        $result = $this->db->select("'Pharmacy' as module,patients.patient_name,patients.id as patient_id,pharmacy_bill_basic.id as bill_no,pharmacy_bill_basic.net_amount as charge")
            ->join("patients", "patients.id = pharmacy_bill_basic.patient_id", "left")
            ->where("pharmacy_bill_basic.case_reference_id", $case_reference_id)
            ->get("pharmacy_bill_basic")
            ->result();
        foreach ($result as $key => $value) {
            $result[$key]->payments = $this->getPatientPharmacyPayments($value->bill_no);
        }
        return $result;
    }
    ...
    public function get_patientidbyopdid($id){
        return $this->db->select('patient_id,visit_details.opd_details_id,cons_doctor as doctor_id,case_reference_id,cons_doctor,staff.name as doctor_name,staff.surname as doctor_surname,staff.employee_id as doctor_employee_id')
        ->from('visit_details')
        ->join('opd_details','opd_details.id=visit_details.opd_details_id')
        ->join('staff','staff.id=visit_details.cons_doctor')
    ...
    public function getipdoverviewtreatment($patient_id)
    {
        $userdata           = $this->customlib->getUserData();        
        $this->db
            ->select('patients.*,bed.name as bed_name,bed_group.name as bedgroup_name, floor.name as floor_name,ipd_details.date,ipd_details.id as ipdid,ipd_details.case_reference_id,ipd_details.credit_limit as ipdcredit_limit,ipd_details.case_type,ipd_details.symptoms,staff.name,staff.surname,staff.employee_id,generated_by_staff.name as generated_byname,generated_by_staff.surname as generated_bysurname,generated_by_staff.employee_id as generated_byemployee_id')
            ->join('patients', 'patients.id = ipd_details.patient_id', "inner")
            ->join('staff', 'staff.id = ipd_details.cons_doctor', "inner")
            ->join('staff as generated_by_staff', 'generated_by_staff.id = ipd_details.generated_by', "left")
            ->join('bed', 'ipd_details.bed = bed.id', "left")
    ...
    public function getallopdvisits($type, $show_antenatal = false, $caller = 'bill_opd')
    {
        $userdata           = $this->customlib->getUserData();
        $doctor_restriction = $this->session->userdata['hospitaladmin']['doctor_restriction'];
        ...
        // Orderable list MUST match each caller view's <th> order 1:1.
        // This model is shared by TWO views with different column layouts:
        //   - bill_opd      → application/views/admin/bill/opd/opd.php
        //   - patient_search → application/views/admin/patient/search.php (OPD tab)
        // Mixing one list across both causes silent wrong-sort or "Invalid JSON" crashes.
        if ($caller === 'patient_search') {
            $orderable_cols = array(
                'opd_details.id',                 // [0] OPD No
                'patients.patient_name',          // [1] Patient Name
                'opd_details.case_reference_id',  // [2] Case Id
                'visit_details.appointment_date', // [3] Appointment Date
                'generated_by_staff.name',        // [4] Generated By
                'staff.name',                     // [5] Consultant
                'visit_details.refference',       // [6] Reference
                'visit_details.symptoms',         // [7] Symptoms
            );
            if ($show_antenatal) {
                $orderable_cols[] = 'visit_details.is_antenatal';  // RBAC-gated (opd_antenatal.can_view)
            }
            if (!empty($custom_field_column_array)) {
                $orderable_cols = array_merge($orderable_cols, $custom_field_column_array);
            }
        } else {
            $orderable_cols = array(
                'patients.patient_name',          // [0] Name
                'opd_details.patient_id',         // [1] Patient Id
                'patients.guardian_name',         // [2] Guardian Name
                'patients.gender',                // [3] Gender
                'patients.mobileno',              // [4] Phone
                'generated_by_staff.name',        // [5] Generated By
                'staff.name',                     // [6] Consultant Doctor
                'visit_details.appointment_date', // [7] Last Visit (date)
            );
            if (!empty($custom_field_column_array)) {
                $orderable_cols = array_merge($orderable_cols, $custom_field_column_array);
            }
            $orderable_cols[] = 'total_visit';                     // Total Visit (count alias from SELECT)
            if ($show_antenatal) {
                $orderable_cols[] = 'visit_details.is_antenatal';  // RBAC-gated (antenatal.can_view)
            }
        }
        // Action column is excluded — auto-non-sortable (initDatatable default columnDefs targets:[-1]).
        $orderable_string = implode(',', $orderable_cols);
		if ($doctor_restriction == 'enabled') {
            if ($userdata["role_id"] == 3) {
		...
		$nextDay     = date('Y-m-d', strtotime('now + 1day'));
			...
			if($type == 1){
				$this->datatables->where('visit_details.appointment_date >=',  $toDay );
				$this->datatables->where('visit_details.appointment_date <',  $nextDay );
			}elseif($type == 2){
				$this->datatables->where('visit_details.appointment_date >',  $nextDay );
			}elseif($type == 3){
				$this->datatables->where('visit_details.appointment_date <',  $toDay );
        ...
        $this->datatables
            ->select('opd_details.case_reference_id,opd_details.id as opd_id,opd_details.patient_id as patientid,opd_details.is_ipd_moved,max(visit_details.id) as visit_id,visit_details.appointment_date,visit_details.refference,visit_details.symptoms,patients.id as pid,patients.patient_name,staff.id as staff_id,staff.name,staff.surname,staff.employee_id,consult_charges.standard_charge,patient_charges.apply_charge,visit_details.is_antenatal,generated_by_staff.name as generated_byname,generated_by_staff.surname as generated_bysurname,generated_by_staff.employee_id as generated_byemployee_id,patients.gender,patients.guardian_name,patients.mobileno,max(visit_details.appointment_date) as last_visit,count(opd_details.patient_id) as total_visit,' . $field_variable)
            ->join('visit_details', 'opd_details.id = visit_details.opd_details_id')
            ->join('staff', 'staff.id = visit_details.cons_doctor', "inner")
            ->join('staff as generated_by_staff', 'generated_by_staff.id = opd_details.generated_by', "left")
            ->join('patients', 'patients.id = opd_details.patient_id', "inner")
        ...
        $result = $this->datatables->generate('json');    //echo $this->db->last_query(); die;
        return $result;
    }

    public function getOpdSidebarList($type, $page = 1, $per_page = 10, $search = '')
    {
        $toDay   = date('Y-m-d', strtotime('now'));
        $nextDay = date('Y-m-d', strtotime('now + 1day'));
        $offset  = ($page - 1) * $per_page;

        // Count total unique OPD records for pagination
        $this->db->select('COUNT(DISTINCT visit_details.opd_details_id) as total', FALSE);
        $this->db->from('opd_details');
        $this->db->join('visit_details', 'opd_details.id = visit_details.opd_details_id', 'inner');
        $this->db->join('patients', 'patients.id = opd_details.patient_id', 'inner');
        if ($type == 1) {
            $this->db->where('visit_details.appointment_date >=', $toDay);
            $this->db->where('visit_details.appointment_date <', $nextDay);
        } elseif ($type == 2) {
            $this->db->where('visit_details.appointment_date >', $nextDay);
        } elseif ($type == 3) {
            $this->db->where('visit_details.appointment_date <', $toDay);
        ...
        if (!empty($search)) {
            $this->db->group_start();
            $this->db->like('opd_details.id', $search);
            $this->db->or_like('patients.id', $search);
            $this->db->or_like('patients.patient_name', $search);
            $this->db->group_end();
        }
        $count_row = $this->db->get()->row_array();
        $total     = !empty($count_row['total']) ? (int) $count_row['total'] : 0;

        // Paginated data
        $this->db->select('opd_details.id as opd_id, opd_details.patient_id, patients.patient_name, max(visit_details.appointment_date) as appointment_date');
        $this->db->from('opd_details');
        $this->db->join('visit_details', 'opd_details.id = visit_details.opd_details_id', 'inner');
        $this->db->join('patients', 'patients.id = opd_details.patient_id', 'inner');
        if ($type == 1) {
        ...
        $this->db->group_by('visit_details.opd_details_id');
        $this->db->order_by('visit_details.appointment_date', 'desc');
        $this->db->limit($per_page, $offset);
        $patients = $this->db->get()->result_array();

        return array('patients' => $patients, 'total' => $total, 'page' => $page, 'per_page' => $per_page);
    }

    public function getPatientSidebarList($page = 1, $per_page = 12, $search = '')
    {
        $offset = ($page - 1) * $per_page;

        // Count total patients
        $this->db->select('COUNT(DISTINCT patients.id) as total', FALSE);
        $this->db->from('patients');
        $this->db->join('opd_details', 'opd_details.patient_id = patients.id', 'inner');
        if (!empty($search)) {
            $this->db->group_start();
            $this->db->like('patients.id', $search);
            $this->db->or_like('patients.patient_name', $search);
            $this->db->group_end();
        }
        $count_row = $this->db->get()->row_array();
        ...
        $this->db->select('patients.id as patient_id, patients.patient_name, max(opd_details.id) as last_opd_id');
        $this->db->from('patients');
        $this->db->join('opd_details', 'opd_details.patient_id = patients.id', 'inner');
        if (!empty($search)) {
            $this->db->group_start();
        ...
        $this->db->order_by('last_opd_id', 'desc');
        $this->db->limit($per_page, $offset);
        $patients = $this->db->get()->result_array();
    ...
    public function getIpdSidebarList($page = 1, $per_page = 12, $search = '', $bed_group_id = 0)
    {
        $offset = ($page - 1) * $per_page;

        // Count total active IPD records
        $this->db->select('COUNT(ipd_details.id) as total', FALSE);
        $this->db->from('ipd_details');
        $this->db->join('patients', 'patients.id = ipd_details.patient_id', 'inner');
        $this->db->join('bed', 'ipd_details.bed = bed.id', 'left');
        $this->db->where('ipd_details.discharged', 'no');
        $this->db->where('patients.is_active', 'yes');
        if (!empty($bed_group_id)) {
            $this->db->where('ipd_details.bed_group_id', $bed_group_id);
        }
        if (!empty($search)) {
            $this->db->group_start();
            $this->db->like('ipd_details.id', $search);
            $this->db->or_like('patients.patient_name', $search);
            $this->db->or_like('bed.name', $search);
            $this->db->group_end();
        }
        $count_row = $this->db->get()->row_array();
        $total     = !empty($count_row['total']) ? (int) $count_row['total'] : 0;
        ...
        // Paginated IPD list
        $this->db->select('ipd_details.id as ipd_id, patients.patient_name, patients.id as patient_id, bed.name as bed_name, bed_group.name as bed_group_name');
        $this->db->from('ipd_details');
        $this->db->join('patients', 'patients.id = ipd_details.patient_id', 'inner');
        $this->db->join('bed', 'ipd_details.bed = bed.id', 'left');
        $this->db->join('bed_group', 'ipd_details.bed_group_id = bed_group.id', 'left');
