...
class Cashfree extends Patient_Controller
{
    ...
    public function index() {
        ...
        $data['gateway_processing_charge']   = $payment_data['gateway_processing_charge'];
        $data['charge_type']                 = $payment_data['charge_type'];
    ...
    public function pay()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $amount =number_format((float)($payment_data['deposit_amount']+$payment_data['gateway_processing_charge']), 2, '.', '');
            ...
            $ch = curl_init();
            //curl_setopt($ch, CURLOPT_URL, 'https://sandbox.cashfree.com/pg/orders'); //testing 
             curl_setopt($ch, CURLOPT_URL, 'https://api.cashfree.com/pg/orders'); //for live
    ...
    public function success()
    {
        ...
        $ch = curl_init();
        //curl_setopt($ch, CURLOPT_URL, 'https://sandbox.cashfree.com/pg/orders'.$_GET['order_id']); //for testing 
         curl_setopt($ch, CURLOPT_URL, 'https://api.cashfree.com/pg/orders/'.$_GET['order_id']); //for live
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
       ...
       if (isset($gateway_response->order_status) && $gateway_response->order_status=="PAID") {
            ...
            $save_record = array(
                    "processing_charge_type"    => $payment_data["charge_type"],
                    "gateway_processing_charge" => $payment_data["gateway_processing_charge"],
