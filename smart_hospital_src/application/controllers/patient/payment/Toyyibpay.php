...
class Toyyibpay extends Patient_Controller {
    ...
    public function pay(){
        ...
        $data["gateway_processing_charge"]  = $session_data['gateway_processing_charge'] ;

        $result = array();
        $this->form_validation->set_rules('phone', $this->lang->line('phone'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('email', $this->lang->line('email'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            $data['amount'] =number_format((float)($session_data['deposit_amount']+$session_data['gateway_processing_charge']), 2, '.', '');
            ...
            $amount =number_format((float)($session_data['deposit_amount']+$session_data['gateway_processing_charge']), 2, '.', ''); 
            $payment_data = array(
                ...
                'billPriceSetting'=>1,
                'billPayorInfo'=>1,				
                'billAmount'=>($session_data['deposit_amount']+$session_data['gateway_processing_charge'])*100,				
            ...
            if (!empty($obj)) {
                ...
                $save_record = array(
                    "processing_charge_type"    =>  $session_data["charge_type"],
                    "gateway_processing_charge" =>  $session_data["gateway_processing_charge"],
